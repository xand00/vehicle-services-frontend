...
const VehicleItemInput = ({ vehicleBrands, inputKeyToErrorList, touchedInputs }) => {
    ...
    const isVehicleBrandErrorListRendered = getIsErrorListRendered(inputKeyToErrorList, touchedInputs, 'vehicle_brand');
    const isVehicleModelErrorListRendered = getIsErrorListRendered(inputKeyToErrorList, touchedInputs, 'vehicle_model');
            ...
            >
                <select hidden ref={vehicleBrandRef} name="vehicle_brand">
                    {vehicleBrandsWithUnselectedOption.map(vehicleBrand => (
                        <option key={JSON.stringify(vehicleBrand)} value={vehicleBrand.id}></option>
                    ))}
                </select>
                <Listbox
                value={selectedVehicleBrand}
                name="vehicle_brand"
                onChange={value => { vehicleBrandRef.current.value = value.id; vehicleBrandRef.current.dispatchEvent(new Event('change', { bubbles: true })); customSetSelectedVehicleBrand(value); }}
                >
                {({ open }) => (
                    <>
                    <div className="mt-1 relative">
                        <Listbox.Button className={`${isVehicleBrandErrorListRendered ? 'invalid-input ' : ''}relative w-full bg-white border border-gray-300 border-2 shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-tertiary focus:bg-tertiary sm:text-sm`}>
                        ...
                        <Listbox.Options className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-56 py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                            {vehicleBrandsWithUnselectedOption.map(
                            (vehicleBrand) => (
                                <Listbox.Option
                                key={vehicleBrand.id}
                                className={({ active }) =>
            ...
            >
                <select hidden ref={vehicleModelRef} name="vehicle_model">
                    {vehicleModelsWithUnselectedOption.map(vehicleModel => (
                        <option key={JSON.stringify(vehicleModel)} value={vehicleModel.id}></option>
                    ))}
                </select>
                <Listbox
                value={selectedVehicleModel}
                name="vehicle_model"
                onChange={value => { vehicleModelRef.current.value = value.id; vehicleModelRef.current.dispatchEvent(new Event('change', { bubbles: true })); setSelectedVehicleModel(value); }}
                >
                {({ open }) => (
                    <>
                    <div className="mt-1 relative">
                        <Listbox.Button className={`${isVehicleModelErrorListRendered ? 'invalid-input ' : ''}relative w-full bg-white border border-gray-300 border-2 shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-tertiary focus:bg-tertiary sm:text-sm`}>
                        <span className="flex items-center">
                        ...
                        >
                        <Listbox.Options className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-56 py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                            {vehicleModelsWithUnselectedOption.map(
                            (vehicleModel) => (
                                <Listbox.Option
                                key={vehicleModel.id}
                                className={({ active }) =>

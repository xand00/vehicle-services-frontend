...
const Footer = () => {
  ...
  const [fullPhoneNumberForLink, setFullPhoneNumberFotLink] = useState(() => null);
  useEffect(async () => {
    try {
      const contact = await getContact();
      const parsedPhoneNumber = parsePhoneNumber(contact.fullPhoneNumber.replace(/\D/g, ""), 'RU');
      setContact(contact);
      if(parsedPhoneNumber) {
        setFullPhoneNumberFotLink(parsedPhoneNumber.number)
      }
    } catch (error) {
          ...
          <div className="my-5">
            <div>
              <a href={'tel:' + fullPhoneNumberForLink} className="block link">
                ...
                <span>{contact?.fullPhoneNumber ?? '-'}</span>
              </a>

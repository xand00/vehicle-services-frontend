...
const Email: React.FC<EmailProps> = ({ color = 'black', isTextHiddenOnMobile = false }) => {
	...
	return (
		<a href={"mailto:" + contactInfo.email} className="block link">
			...
			<span className={isTextHiddenOnMobile ? 'hidden md:inline-block' : ''}> {contactInfo.email ?? "-"}</span>
		</a>
	)
}

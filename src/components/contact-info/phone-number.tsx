...
const PhoneNumber: React.FC<PhoneNumberProps> = ({ color = 'black', isTextHiddenOnMobile = false }) => {
	...
	return (
		<a href={"tel:" + fullPhoneNumberForLink} className="block link max-w-max">
			<svg
			...
			<span className={`${isTextHiddenOnMobile ? 'hidden md:inline-block' : ''}`}>{contactInfo.fullPhoneNumber ?? "-"}</span>
		</a>
	)
}
